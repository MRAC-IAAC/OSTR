...
#from aslinur, need for using picamera and taking picture
...
#from aslinur
pushbutton = 4 #define the pin where the touch sensor signal is coming from
camera = PiCamera() #define what camera means
...
#from aslinur: to take picture with touch sensor
def take_picture_with_button():
    image_path = '/home/pi/Desktop/SHEDIO/SavedImages/a_%s.jpg' % int(round(time.time() * 1000))
    camera.capture(image_path)
    print('I took a photo')


def recall_image():
    list_of_files = glob.glob('/home/pi/Desktop/SHEDIO/SavedImages/*.jpg')  # path to specific folder
...
button.when_pressed = take_picture_with_button
time.sleep(10) #Wait 10 seconds

print("I'm locating the picture taken")

ledwhite_off()

#Recal latest image
img_path = recall_image()
